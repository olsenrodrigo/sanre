...
export default function CheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <main className="campos-retos container-sr max-w-5xl py-10 md:py-14">
          ...
          {steps.map((s, i) => (
            <div key={s.n} className="flex items-center">
              <div className="flex flex-col items-center">
                <div className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold transition-colors ${step >= s.n ? "bg-sr-ink text-sr-paper" : "bg-sr-nude-50 text-sr-ink-soft"}`}>
                  {step > s.n ? <Check size={14} /> : s.n}
                </div>
                <span className={`mt-1 hidden text-xs sm:block ${step >= s.n ? "font-medium text-sr-ink" : "text-sr-ink-soft"}`}>{s.label}</span>
              </div>
              {i < steps.length - 1 && <div className={`mx-1 mb-4 h-0.5 w-8 transition-colors sm:mb-0 ${step > s.n ? "bg-sr-nude-600" : "bg-sr-nude-100"}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2">
            <div className=" border border-sr-nude-100 bg-card p-6 shadow-[0_1px_2px_rgb(52_55_46/0.05)] md:p-8">

              {/* Step 1: Identity */}
              {step === 1 && (
                <div>
                  <h2 className="mb-5 text-2xl text-sr-ink">Suas informações</h2>
                  <div className="space-y-4">
                    <div>
                      <Label htmlFor="checkout-name">Nome completo *</Label>
                      <Input id="checkout-name" value={identity.name} onChange={e => setIdentity(i => ({...i, name: e.target.value}))} autoComplete="name" className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                    </div>
                    <div>
                      <Label htmlFor="checkout-email">E-mail *</Label>
                      <Input id="checkout-email" type="email" value={identity.email} onChange={e => setIdentity(i => ({...i, email: e.target.value}))} autoComplete="email" className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="checkout-phone">Telefone / WhatsApp *</Label>
                        <Input id="checkout-phone" value={identity.phone} onChange={e => setIdentity(i => ({...i, phone: e.target.value}))} autoComplete="tel" className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                      </div>
                      <div>
                        <Label htmlFor="checkout-cpf">CPF *</Label>
                        <Input id="checkout-cpf" value={identity.cpf} onChange={e => setIdentity(i => ({...i, cpf: e.target.value}))} className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                      </div>
                    </div>
                    <label htmlFor="recover-consent" className="flex cursor-pointer items-start gap-2.5 text-sm text-sr-ink-soft">
                      <input
                        id="recover-consent"
                        type="checkbox"
                        ...
                        className="mt-0.5 h-5 w-5 accent-sr-nude-600"
                      />
              ...
              {step === 2 && (
                <div>
                  <h2 className="mb-5 text-2xl text-sr-ink">Endereço de entrega</h2>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input
                          ...
                          className="mt-1 min-h-11 border-sr-nude-200 bg-card"
                          maxLength={9}
                        />
                        {loadingCep && <p className="mt-1 text-sm text-sr-ink-soft" aria-live="polite">Buscando CEP...</p>}
                      </div>
                    </div>
                    <div>
                      <Label htmlFor="checkout-street">Logradouro *</Label>
                      <Input id="checkout-street" value={address.logradouro} onChange={e => setAddress(a => ({...a, logradouro: e.target.value}))} autoComplete="street-address" className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="checkout-number">Número *</Label>
                        <Input id="checkout-number" value={address.numero} onChange={e => setAddress(a => ({...a, numero: e.target.value}))} className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                      </div>
                      <div>
                        <Label htmlFor="checkout-complement">Complemento</Label>
                        <Input id="checkout-complement" value={address.complemento} onChange={e => setAddress(a => ({...a, complemento: e.target.value}))} className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                      </div>
                    </div>
                    <div>
                      <Label htmlFor="checkout-neighborhood">Bairro *</Label>
                      <Input id="checkout-neighborhood" value={address.bairro} onChange={e => setAddress(a => ({...a, bairro: e.target.value}))} className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="checkout-city">Cidade *</Label>
                        <Input id="checkout-city" value={address.cidade} onChange={e => setAddress(a => ({...a, cidade: e.target.value}))} autoComplete="address-level2" className="mt-1 min-h-11 border-sr-nude-200 bg-card" />
                      </div>
                      <div>
                        ...
                        <select id="checkout-state" value={address.estado} onChange={e => setAddress(a => ({...a, estado: e.target.value}))}
                          className="mt-1 min-h-11 w-full  border border-sr-nude-200 bg-card px-3 text-sr-ink">
              ...
              {step === 3 && (
                <div>
                  <h2 className="mb-5 text-2xl text-sr-ink">Entrega ou retirada</h2>
                  <p className="nav-label mb-3 text-sr-ink-soft">Retirar numa loja Sanrê — grátis</p>
                  <div className="mb-6 space-y-3">
                    {UNIDADES.map(u => {
                      const selected = shipping.carrier === `loja:${u.slug}`;
                      return (
                        <label key={u.slug} className={`flex min-h-14 cursor-pointer items-center gap-3 border p-3 transition-colors ${selected ? "border-2 border-sr-ink bg-white" : "border-sr-line hover:border-sr-nude-400"}`}>
                          <input
                            type="radio"
                            name="shipping"
                            checked={selected}
                            onChange={() => setShipping({ carrier: `loja:${u.slug}`, service: `Retirada — ${u.cidade}`, amount: 0 })}
                            className="h-5 w-5 accent-[#141414]"
                          />
                          <div className="flex-1">
                            <p className="text-sm font-medium text-sr-ink">Retirar em {u.cidade}</p>
                            <p className="text-sm text-sr-ink-soft">{u.logradouro}{u.complemento ? ` · ${u.complemento}` : ""} — avisamos pelo WhatsApp quando estiver separado</p>
                          </div>
                          <span className="font-semibold text-sr-ok">Grátis</span>
                        </label>
                      );
                    })}
                  </div>
                  <p className="nav-label mb-3 text-sr-ink-soft">Receber em casa</p>
                  <div className="space-y-3">
                    {shipLoading && (
                      <p className="text-sm text-sr-ink-soft" aria-live="polite">Calculando frete...</p>
                    )}
                    {shipOptions && shipOptions.length === 0 && !shipLoading && (
                      <p className="text-sm text-sr-alert" aria-live="polite">Nenhuma opção de frete para este CEP.</p>
                    )}
                    {(shipOptions || []).map(opt => {
                      const selected = !shipping.carrier.startsWith("loja:") && shipping.service === opt.service;
                      const price = opt.free ? 0 : opt.finalValue;
                      return (
                        <label key={opt.id + opt.service} className={`flex min-h-14 cursor-pointer items-center gap-3  border p-3 transition-colors ${selected ? "border-2 border-sr-nude-600 bg-sr-nude-50" : "border-sr-nude-200 hover:border-sr-nude-300"}`}>
                          <input type="radio" name="shipping"
                            checked={selected}
                            onChange={() => setShipping({ carrier: opt.base || "SmartEnvios", service: opt.service, amount: price })}
                            className="h-5 w-5 accent-sr-nude-600"
                          />
                          <div className="flex-1">
                            <p className="text-sm font-medium text-sr-ink">{opt.service}</p>
                            <p className="text-sm text-sr-ink-soft">{opt.days} dias úteis</p>
                          </div>
              ...
              {step === 4 && (
                <div>
                  <h2 className="mb-5 text-2xl text-sr-ink">Forma de pagamento</h2>
                  <div className="space-y-3 mb-4">
                    {[
                      ...
                      <label key={opt.method} className={`flex min-h-14 cursor-pointer items-center gap-3  border p-3 transition-colors ${paymentMethod === opt.method ? "border-2 border-sr-nude-600 bg-sr-nude-50" : "border-sr-nude-200 hover:border-sr-nude-300"}`}>
                        <input type="radio" name="payment" value={opt.method} checked={paymentMethod === opt.method}
                          onChange={() => setPaymentMethod(opt.method)} className="h-5 w-5 accent-sr-nude-600" />
                        <span className="text-sr-nude-600">{opt.icon}</span>
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-medium text-sr-ink">{opt.label}</p>
                            {opt.badge && <span className="bg-sr-nude-100 px-2 py-0.5 text-xs text-sr-nude-700">{opt.badge}</span>}
                          </div>
                          <p className="text-sm text-sr-ink-soft">{opt.desc}</p>
                        </div>
                      </label>
                    ))}
                  </div>
                  ...
                  <div className="mb-4  bg-alt p-4">
                    <Label htmlFor="coupon-code" className="mb-2 block text-sm font-medium text-sr-ink">Cupom de desconto</Label>
                    <div className="flex gap-2">
                      <Input id="coupon-code" value={couponCode} onChange={e => setCouponCode(e.target.value.toUpperCase())} className="min-h-11 flex-1 border-sr-nude-200 bg-card" />
                      <Button type="button" variant="outline" className="btn-line" onClick={() => applyCoupon()} disabled={couponLoading}>Aplicar</Button>
                    </div>
                    {couponDiscount > 0 && <p className="mt-2 text-sm text-sr-nude-700" aria-live="polite">✓ Desconto de {precoBR(couponDiscount)} aplicado!</p>}
                  </div>
                  ...
                  {storeInfo.contactWhatsapp && (
                    <>
                      <div className="my-3 flex items-center gap-3 text-xs text-sr-ink-soft">
                        <span className="h-px flex-1 bg-sr-nude-200" /> ou <span className="h-px flex-1 bg-sr-nude-200" />
                      </div>
                      <Button
                        type="button"
          ...
          {/* Order summary */}
          <aside className="sticky top-[calc(var(--sr-header)+1.5rem)] h-fit  bg-alt p-5">
            <h3 className="mb-4 font-sans text-lg font-semibold text-sr-ink">Resumo</h3>
            <div className="space-y-2 text-sm">
              {cart?.items.map(item => (
                <div key={item.id} className="flex gap-2">
                  <div className="aspect-fashion w-12 flex-shrink-0 overflow-hidden  bg-sr-nude-50">
                    {item.mainImage && <img src={item.mainImage} alt={item.productTitle} className="h-full w-full object-cover" />}
                  </div>
                  <div className="flex-1 text-xs">
                    <p className="line-clamp-1 text-sr-ink">{item.productTitle}</p>
                    {item.variantTitle && <p className="text-sr-ink-soft">{item.variantTitle}</p>}
                    <p className="text-sr-ink-soft">× {item.quantity}</p>
                  </div>
                  <p className="text-xs font-medium text-sr-ink">{precoBR(Number(item.unitPrice) * item.quantity)}</p>
                </div>
              ))}
              <div className="space-y-1 border-t border-sr-nude-200 pt-2">
                <div className="flex justify-between text-sr-ink-soft"><span>Subtotal</span><span>{precoBR(total)}</span></div>
                {couponDiscount > 0 && <div className="flex justify-between text-sr-nude-700"><span>Desconto</span><span>- {precoBR(couponDiscount)}</span></div>}
                <div className="flex justify-between text-sr-ink-soft"><span>Frete</span><span>{shipping.amount > 0 ? precoBR(shipping.amount) : "-"}</span></div>
                {paymentMethod === "pix" && <div className="flex justify-between text-xs text-sr-nude-700"><span>Desconto PIX ({Math.round(PIX_DESCONTO * 100)}%)</span><span>- {precoBR(pixDesconto)}</span></div>}
              </div>
